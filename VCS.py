...
#     Scene, Text, Underline, Create, Write, VGroup, np, VMobject,
    
#     # Geometric objects
#     Line, Triangle, Rectangle, Arrow, Brace, Polygon, Circle,
...
#         semantic_text = Text("Global Alignment Score (GAS)", font_size=16).set_color_by_gradient(RED, BLUE)
...
#         content_text = Text("Local Alignment Score (LAS)", font_size=16).set_color_by_gradient(RED, BLUE)
...
#         scale_text = MathTex(r"GAS - (1 - LAS) > 0", font_size=14)
...
#         new_box_text = MathTex(r"Semantic Alignment Score (SAS)", font_size=15)
#         new_box_rect = Rectangle(width=2.5, height=0.7).move_to(new_box_text.get_center())
...
#         ssas_nas_text = MathTex(r"SAS <= NAS", font_size=13)
...
#         ssas_nas_test_text = MathTex(r"SAS - (1 - NAS)>0", font_size=12)
...
#         result_text = MathTex(r"VCS^*", font_size=19)
...
#         nas_test_text = MathTex(r"NAS - (1 - SAS)>0", font_size=12)
...
#         result_text_nas = MathTex(r"VCS^*", font_size=19)
...
#         final_vad_text = MathTex(r"VCS^*", font_size=19)
...
#         sas_abbrev = Text("GAS", font_size=16).set_color_by_gradient(RED_A)
#         cas_abbrev = Text("LAS", font_size=16).set_color_by_gradient(RED_A)
...
from manim import (  # type: ignore
    # Core scene and animation classes
    Scene, Text, Underline, Create, Write, VGroup, np, VMobject, Circle,
...
class FullConceptAnimation(Scene):
    ...
    def construct(self):
        ...
        # Group all alignment score components for layout
        three_scores_group = VGroup(semantic_group, content_group, narrative_group)
        three_scores_group.arrange(DOWN, aligned_edge=LEFT, buff=0.7).to_edge(LEFT)
       ...
       # Calculate where the box will be for arrow positioning
        ...
        # ================== FINAL VCS AGGREGATION ==================
        # Create vertical brace that wraps both VAD components
        final_brace = Brace(
            VGroup(result_group, result_group_nas),  # Group containing both VCS* boxes
            direction=RIGHT                         # Brace appears on right side
        )
        ...
        # Create the final VCS box with text
        final_vcs_text = MathTex(r"VCS^*", font_size=19)
        final_vcs_box = Rectangle(width=1.0, height=0.455).set_stroke(width=1, color=WHITE)
        final_vcs_text.move_to(final_vcs_box.get_center())
        final_vcs_group = VGroup(final_vcs_box, final_vcs_text)

        # Position final VCS box to the right of the brace
        final_vcs_group.next_to(final_brace, RIGHT*1.38, buff=0.5)
        final_vcs_group.set_y(final_brace.get_center()[1])  # Center vertically

        # Animate the brace
        self.play(Create(final_brace), run_time=0.5)
        self.wait(0.5)

        # Create arrow from brace to final VCS box
        arrow_final_brace_to_vcs = Arrow(
            start=final_brace.get_right(),
            end=final_vcs_box.get_left(),
            buff=0.05,
            color=YELLOW,
            stroke_width=8,  # Thicker line to match other arrows
            max_tip_length_to_length_ratio=0.25,
        ...
        # Animate the arrow
        self.play(GrowArrow(arrow_final_brace_to_vcs), run_time=0.5)
        self.wait(0.5)

        # Animate the final VCS box and text
        self.play(Create(final_vcs_box), Write(final_vcs_text), run_time=0.7)
        self.wait(1.0)
        ...
        # Create new text for "Scaled.SAS = 0.82"
        scaled_sas_value = MathTex(r"SAS = 0.82", font_size=13)
        scaled_sas_value.move_to(new_box_text.get_center())
        ...
        # -------------------------------------------------------------
        # Now animate the VCS* box with simplified effect (just the moving outline)
        # -------------------------------------------------------------

        # Get a reference to the VCS* box
        vcs_star_box = result_box  # Reference to the existing VCS* box
        vcs_star_text = result_text  # Reference to the existing VCS* text

        # Get the center position of the VCS* box
        vcs_star_center = vcs_star_text.get_center()

        # Create new text "VCS* = 0.78" with smaller font to fit in the box
        vcs_value_text = MathTex(r"VCS^* = 0.78", font_size=13)  # Reduced from 19 to 13
        vcs_value_text.move_to(vcs_star_center)

        # Create tracker for the VCS* box
        vcs_star_tracker = ValueTracker(formula_result_tracker.get_value())  # Start synchronized with the formula result

        # Create moving gradient outline for the VCS* box
        vcs_star_outline = create_moving_outline(vcs_star_box, vcs_star_tracker)

        # Add the outline to the scene
        self.add(vcs_star_outline)

        # Update the updater function to include the VCS* box
        self.updaters = []
        ...
        def update_final_trackers(dt):
            ...
            # Keep glowing effects in sync
            formula_result_tracker.increment_value(dt * 0.4)
            vcs_star_tracker.increment_value(dt * 0.4)  # Same speed as result_tracker
        ...
        # Animate the transformation of VCS* to VCS* = 0.78
        self.play(
            Transform(vcs_star_text, vcs_value_text),
            run_time=1.0
        )

        # Final wait to show the completed animation - ending at VCS* = 0.78
        self.wait(3.0)
        ...
        def update_all_trackers(dt):
            ...
            result_tracker.increment_value(dt * 0.4)
            formula_result_tracker.increment_value(dt * 0.4)
            vcs_star_tracker.increment_value(dt * 0.4)
        ...
        # Create a tracker for the VCS** box (the second VCS* box)
        vcs_star2_box = result_box_nas  # Reference to the existing second VCS* box
        vcs_star2_text = result_text_nas  # Reference to the existing VCS* text

        # Create tracker for the VCS** box
        vcs_star2_tracker = ValueTracker(vcs_star_tracker.get_value())  # Start synchronized

        # Create moving gradient outline for the VCS** box
        vcs_star2_outline = create_moving_outline(vcs_star2_box, vcs_star2_tracker)

        # Add the outline to the scene
        self.add(vcs_star2_outline)

        # Update the updater function to include both VCS* boxes
        self.updaters = []
        ...
        def update_complete_trackers(dt):
            ...
            formula_result_tracker.increment_value(dt * 0.4)
            vcs_star_tracker.increment_value(dt * 0.4)
            vcs_star2_tracker.increment_value(dt * 0.4)  # Add new tracker
            nas_formula_result_tracker.increment_value(dt * 0.4)
        ...
        # Get the center position of the VCS** box
        vcs_star2_center = vcs_star2_text.get_center()

        # Create new text "VCS* = 0.81" with matching font size
        vcs_star2_value_text = MathTex(r"VCS^* = 0.81", font_size=13)
        vcs_star2_value_text.move_to(vcs_star2_center)

        # Wait a moment before starting the transformation
        self.wait(1.0)

        # Animate the transformation of VCS** to VCS* = 0.81
        self.play(
            Transform(vcs_star2_text, vcs_star2_value_text),
            run_time=1.0
        )
        ...
        # Create a tracker for the Final VCS box
        final_vcs_tracker = ValueTracker(vcs_star2_tracker.get_value())  # Start synchronized with other VCS boxes

        # Create moving gradient outline for the Final VCS box
        final_vcs_outline = create_moving_outline(final_vcs_box, final_vcs_tracker)

        # Add the outline to the scene
        self.add(final_vcs_outline)

        # Update the updater function to include the Final VCS box
        self.updaters = []
        ...
        def update_final_complete_trackers(dt):
            ...
            formula_result_tracker.increment_value(dt * 0.4)
            vcs_star_tracker.increment_value(dt * 0.4)
            vcs_star2_tracker.increment_value(dt * 0.4)
            nas_formula_result_tracker.increment_value(dt * 0.4)
            final_vcs_tracker.increment_value(dt * 0.4)  # Add new tracker for Final VCS

        # Add the updated tracker
        self.add_updater(update_final_complete_trackers)

        # Get the center position of the Final VCS box
        final_vcs_center = final_vcs_text.get_center()

        # Create new text "Final VCS = 0.78" with matching font size and gradient color
        final_vcs_value_text = MathTex(r"VCS^* = 0.78", font_size=14)
        final_vcs_value_text.move_to(final_vcs_center)

        # Wait a moment before starting the transformation
        self.wait(1.0)

        # Animate the transformation of Final VCS to Final VCS = 0.78
        self.play(
            Transform(final_vcs_text, final_vcs_value_text),
            run_time=1.0
        )
